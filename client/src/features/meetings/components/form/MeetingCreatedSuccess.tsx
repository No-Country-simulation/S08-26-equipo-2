...
export function MeetingCreatedSuccess({ meeting, onNav }: MeetingCreatedSuccessProps) {
    ...
    <div className="flex-1 overflow-y-auto p-6 flex items-center justify-center">
      <Card className="p-8 max-w-lg w-full text-center border-border bg-card rounded-2xl shadow-2xl">
        ...
        <p className="text-sm mb-6 text-muted-foreground">
          {meeting.name || 'Mi Reunión'} · {meeting.date || '10 sep'} · {meeting.time || '10:00'} · {meeting.duration || '60 min'}
        </p>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6 text-center">
          {[
            ...
            { icon: Users, label: meeting.duration ? `${meeting.duration}` : '60 min' },
          ].map(({ icon: Icon, label }) => (
            <div
              key={label}
...
export default MeetingCreatedSuccess;
